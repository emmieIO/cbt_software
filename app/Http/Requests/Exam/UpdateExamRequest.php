...
class UpdateExamRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('exam:edit');
    }
    ...
    protected function validateAcademicConsistency(Validator $validator): void
    {
        ...
        if (! is_array($compositions) || ! $schoolClass) {
            if ($status === 'live' && empty($startTime)) {
                $validator->errors()->add('start_time', 'A start date and time is required before an examination can go live.');
            }
