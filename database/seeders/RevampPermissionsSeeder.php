...
class RevampPermissionsSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $superAdmin->update(['category' => 'admin']);
        ...
        $staffUser = User::where('email', 'staff@chrisland.org')->first();
        if ($staffUser) {
            $staffUser->assignRole('examiner');
        }

        $studentUser = User::where('email', 'student@chrisland.org')->first();
        if ($studentUser) {
            $studentUser->assignRole('candidate');
        }
    }
}
