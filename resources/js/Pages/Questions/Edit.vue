...
const form = ref({
    ...
    marking_scheme: props.question.marking_scheme?.length ? props.question.marking_scheme : [{ point: '', weight: 1 }],
...
const correctIndex = computed({
    ...
    set: (idx: number) => {
        form.value.options.forEach((o: any, i: number) => {
            o.is_correct = i === idx;
        });
    },
});
...
const submit = () => {
    ...
    if (usesMarkingScheme.value) {
        const scheme = form.value.marking_scheme.filter((s: any) => richText(s.point));
        fd.append('marking_scheme', JSON.stringify(scheme));
    } else {
        if (form.value.options.some((option: any) => !richText(option.content))) {
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-green-900/60 dark:bg-green-950/60 dark:shadow-none">
                    <div class="flex flex-col gap-4 lg:flex-row">
                        ...
                        <label
                            class="flex cursor-pointer items-center gap-3 rounded-lg border-2 p-4 transition-colors"
                            :class="form.type === 'short_answer' ? 'border-primary bg-primary/5' : 'border-gray-200 dark:border-green-900/60'"
                        >
                    ...
                    <div class="mt-4 space-y-4">
                        ...
                        <div>
                            ...
                            <div v-else-if="existingImage" class="mt-2">
                                <img
                                    :src="existingImage"
                                    class="max-h-40 rounded-lg border border-gray-200 object-contain dark:border-green-900/60"
                                />
                            </div>
