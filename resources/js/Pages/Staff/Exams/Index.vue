        ...
        <div class="space-y-6 sm:space-y-10">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500">Manage papers and student schedules • {{ exams.data.length }} Records</p>
                </div>
                <Link
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex flex-col">
                                                ...
                                                <div class="mt-0.5 flex items-center gap-x-2">
                                                    <span v-if="exam.school_id && branches[exam.school_id]" class="text-xs font-medium text-primary">
                                                        {{ branches[exam.school_id].name }}
                                                    </span>
                                                    <span v-if="exam.school_id" class="text-xs text-gray-400">•</span>
                                                    <span class="text-xs text-gray-500">
                                                        {{ exam.school_class?.name || 'General' }}
                                                    </span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex flex-col">
                                                <span class="text-xs text-gray-800"
                                                    ><span class="font-medium">Subject:</span> {{ exam.subject?.name || 'Multi-Subject' }}</span
                                                >
                                                <span class="mt-0.5 text-xs text-gray-500"
                                                    >{{ exam.duration }} Mins • {{ exam.questions_count }} Questions</span
                                                >
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <span
                                                class="inline-flex items-center gap-x-1.5 rounded-md px-3 py-1.5 text-xs font-medium capitalize"
                                        ...
                                        <td class="px-6 py-4 text-end">
                                            <div class="flex items-center justify-end gap-x-2">
                                                ...
                                                <Link
                                                    ...
                                                    class="hover:bg-primary-hover inline-flex items-center gap-x-2 rounded-lg border border-transparent bg-primary px-3 py-1.5 text-xs font-semibold text-white focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:outline-none"
                                                >
                                                    Configure
                                                </Link>
                                            </div>
